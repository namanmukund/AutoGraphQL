    ...
    for (const menteeSession of menteeSessions) {
      ...
      if (slotTimeStringArray && slotTimeStringArray.length) {
        ...
        if (mentorMenteeSession && mentorMenteeSession.id && isB2BBatch) {
          ...
          // send email
          const templateFileName = get(transactionalMessageBody.sendSessionLinkB2B, 'emailTemplate');

          const subject = get(transactionalMessageBody.sendSessionLinkB2B, 'subject');
