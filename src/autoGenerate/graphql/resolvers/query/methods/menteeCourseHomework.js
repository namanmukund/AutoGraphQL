...
const menteeCourseHomework = async (
  ...
  const userCurrentTopicComponentStatusesModel = new QueryController("UserCurrentTopicComponentStatus", {
    bypass: true,
  });
  ...
  const batchCurrentComponentCourseId = get(
    res,
    "data.userCurrentTopicComponentStatuses[0].user.studentProfile.batch.currentComponent.currentCourse.id"
  );
  ...
  log(
    `Time Taken to execute mcsMR : ${(mcsMRTimeStop[0] * 1e9 + mcsMRTimeStop[1]) / 1e9
    } seconds`
  );
