...
const getStoryspreeCertificateUrl = async (userId, userName, formattedDate) => {
  const url = `${process.env.FILE_BASE_URL}/python/course/storyspreeCertificate.pdf`;
  ...
  pdfDoc.setSubject('Tekie\'s Story Spree Certificate');
  pdfDoc.setTitle('Tekie\'s Story Spree Certificate');
  pdfDoc.setProducer('Tekie.in');

  const pdfBytes = await pdfDoc.save();
  const path = '/tmp/storyspree/certificate-pdf.pdf';
  mkdirp.sync('/tmp/storyspree');
  fs.writeFileSync(path, pdfBytes);
  const fileContent = fs.readFileSync(path);
  let fetchedUrl = '';
  if (fileContent) {
    const key = `event-certificate/storyspree/${slugifyID(userId)}-certificate.pdf`;
...
export default getStoryspreeCertificateUrl;
