...
const getChristmasCarnivalCertificateUrl = async (userId, userName, formattedDate) => {
  ...
  pdfDoc.setSubject('Tekie\'s Christmas Carnival Certificate');
  pdfDoc.setTitle('Tekie\'s Christmas Carnival Certificate');
  pdfDoc.setProducer('Tekie.in');

  const pdfBytes = await pdfDoc.save();
  const path = '/tmp/christmascarnival/certificate-pdf.pdf';
  mkdirp.sync('/tmp/christmascarnival');
  fs.writeFileSync(path, pdfBytes);
  const fileContent = fs.readFileSync(path);
  let fetchedUrl = '';
  if (fileContent) {
    const key = `event-certificate/christmascarnival/${slugifyID(userId)}-certificate.pdf`;
...
export default getChristmasCarnivalCertificateUrl;
