...
const getGenZEventCertificateUrl = async (userId, userName, formattedDate) => {
  const url = `${process.env.FILE_BASE_URL}/python/course/genZEnvironmentCertificate.pdf`;
  ...
  pdfDoc.setSubject('Tekie\'s Gen Z for Environment Certificate');
  pdfDoc.setTitle('Tekie\'s  Gen Z for Environment Certificate');
  pdfDoc.setProducer('Tekie.in');

  const pdfBytes = await pdfDoc.save();
  const path = '/tmp/genz/certificate-pdf.pdf';
  mkdirp.sync('/tmp/genz');
  fs.writeFileSync(path, pdfBytes);
  const fileContent = fs.readFileSync(path);
  let fetchedUrl = '';
  if (fileContent) {
    const key = `event-certificate/genz4environment/${slugifyID(userId)}-certificate.pdf`;
...
export default getGenZEventCertificateUrl;
