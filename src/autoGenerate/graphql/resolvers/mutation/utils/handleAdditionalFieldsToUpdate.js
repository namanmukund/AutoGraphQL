...
const handleAdditionalFieldsToUpdate = (input, ast, typeName) => {
  ...
  Object.keys(input)
    .forEach((fieldName) => {
      const { isAdditionalField, originalFieldname, originalFieldValue } =
        isAdditionalFieldName(typeName, ast, fieldName, input);
      if (isAdditionalField) {
        ...
        const additionalFields =
          ast[typeName].field[originalFieldname].directive.relation.argument.fields.value.fields;
