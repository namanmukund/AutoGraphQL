...
import {
  emailTemplates,
  LATO_BOLD_FONT_URL,
  GILROY_EXTRA_BOLD_FONT_URL,
  NUNITO_BOLD_FONT_URL,
} from '../../../../../../../constants';
import { uploadToS3 } from '../../../../../../middlewares/utils';
...
const generateJourneySnapshotUtil = async (templatetoFetch, data, userSavedCodes, userApprovedCodes, userPqCount) => {
  ...
  if (courseName && userName && sessionDate) {
    try {
      const existingPdfBytes = await fetch(templatetoFetch === 'JourneySnapshot-1' ? get(emailTemplates, 'journeySnapshot.journeySnapshot1') : get(emailTemplates, 'journeySnapshot.journeySnapshot2')).then((res) => res.buffer());
      ...
      // get font
      const LatoBoldfontBytes = await fetch(LATO_BOLD_FONT_URL).then((res) => res.buffer());
